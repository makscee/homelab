...
flowchart TB
  ...
  cc-worker-node <-.->|Tailscale| tower-proxmox
  ...
  cc-worker-node -.->|SSH via Tailscale| tower-proxmox
  cc-worker-node -.->|SSH via Tailscale| docker-tower-node
  cc-worker-node -.->|SSH via Tailscale| mcow-node
  cc-worker-node -.->|SSH via Tailscale| nether-node
```
...
| cc-worker | 100.99.133.9 | — | Moscow (LXC 204) | Virtual | Pending SSH query |
...
- **Operator (Claude Code)**: SSH from cc-worker to all servers via Tailscale hostnames (`ssh root@<hostname>`)
...
| cc-worker SPOF | If cc-worker goes down: no Claude Code execution, no deployments until operator machine rebuilt with age key |
