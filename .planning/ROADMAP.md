...
- [x] 12-03-PLAN.md — mcow LXC privilege probe + STATE.md cleanup (obsolete todos closed)
- [x] 12-04-PLAN.md — Auth.js v5 GitHub OAuth + allowlist middleware + CSP/HSTS/XFO nonce headers
...
**Plans**: 5 plans
Plans:
- [x] 14-01-PLAN.md — Ansible provisioning: node-exporter on 6 hosts + cAdvisor on Docker hosts + Prometheus scrape targets
- [x] 14-02-PLAN.md — bun:sqlite audit infrastructure: audit-db.server + logAudit + redactPayload + systemd ReadWritePaths
- [x] 14-03-PLAN.md — /audit RSC paginated viewer + Phase 13 mutation-route migration to logAudit()
- [x] 14-04-PLAN.md — / overview page: 6 host tiles with SWR 30s refresh + degraded/outage states (DASH-01/04/05)
