...
must_haves:
  ...
  artifacts:
    ...
    - path: ".planning/phases/22-security-review-launch/22-02-AUDIT-LOG.md"
      provides: "Recorded evidence from SEC-08 audit scripts"
      contains: "bun audit"
---

## DEFERRED — SEC-01

**Status:** DEFERRED to v3.1 (decision 2026-04-21)

**Rationale:** Stock apt Caddy 2.6.2 on mcow does not include the `rate_limit` module. The
`caddyserver.com/api/download` on-demand build API (Option A) was verified broken today
from two independent egress points (mcow + nether) — both returned 0-byte responses, ~35
minutes wasted. The xcaddy self-build path (Option B) works but adds artifact-management
overhead (CI builder, binary provenance, update cadence). Operator chose launch velocity
over rate-limit; SEC-01 reopens in v3.1.

**Original Task 1 instructions preserved verbatim below for v3.1 pickup:**
...
<objective>
Execute SEC-08 security review surface items that are NOT threat-model aggregation (those go to plan 22-05). This plan covers D-22-07 (bun audit), D-22-08 (bundle scan), D-22-09 (header re-audit).

**SEC-01 (D-22-06, Caddy rate limit) DEFERRED to v3.1** — see DEFERRED section above.

Purpose: Close the code/config portion of SEC-08 before launch. Plan 22-05 handles header-spoofing integration test, Proxmox token scope re-verify, Tailnet-only ingress, and cross-phase SECURITY aggregation.

Output: Three reusable audit scripts; AUDIT-LOG.md capturing evidence for SEC-08 checks.
</objective>
...
<!-- Task 1 (SEC-01 rate limit) DEFERRED to v3.1 — original text preserved in DEFERRED section above -->

<task type="auto" skip="deferred-to-v3.1">
  <name>Task 1 (DEFERRED): SEC-01 — Caddy 60/min per-IP rate limit on auth routes</name>
  <files>
    ansible/playbooks/templates/caddy-homelab-admin.conf.j2,
    ansible/playbooks/deploy-homelab-admin.yml
  </files>
